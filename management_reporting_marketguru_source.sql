...
WHERE
    ...
    --and pp.updated <= to_date('2025-10-15', 'YYYY-MM-DD')
GROUP BY 1,2,3
ORDER BY count(*) desc
)
...
--mg_churn
-- Сортируем и для каждой строки берём максимальную правую границу среди всех предыдущих интервалов.
with sorted as (
    select
        "userId"  user_id,
        "startDate"  start_dt,
...
-- Помечаем начало нового "куска" непрерывного доступа:
-- если start_dt > max_prev_end => новый кусок (иначе продолжаем текущий)
flagged as (
    select
        user_id,
        start_dt,
...
-- Нумеруем куски кумулятивной суммой и затем агрегируем каждый кусок в объединённый интервал
chunked as (
    select
        user_id,
        start_dt,
...
-- Для каждого объединённого периода смотрим на следующий объединённый период и вычисляем разрыв
with_next as (
    select
        m.*,
...
-- Оставляем только разрывы > 30 дней — это отток и последующий возврат
select
	row_number() over (order by user_id) id,
    user_id,
    period_end::date   as churn_date,   -- дата окончания периода, после которой был отток
    next_start::date   as return_date,  -- дата начала следующего периода (возврат)
